...
class UPLOAD_DEVICE_PAYLOAD_2S3 {
	constructor(data) {
		console.log(data);
		if (!data) {
			throw new Error('Invalid UPLOAD_DEVICE_PAYLOAD_2S3 data');
		}
		this.data = { ...data };
	}
	async process() {
		try {
			...
			if (data && data.coreid) {
				...
				await s3FileServer.uploadJsonData(data, {
					bucketName: process.env.DEVICE_PAYLOAD_BUCKET_NAME,
...
module.exports = UPLOAD_DEVICE_PAYLOAD_2S3;
